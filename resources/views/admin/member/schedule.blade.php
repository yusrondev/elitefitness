      ...
      <div class="modal-body">
        ...
        <form action="{{ route('admin.members.storeSchedule') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="hidden" name="idtopup_informasi" id="idtrainer">
                @error('idtrainer')
            ...
            <div class="mb-3">
                <div style="text-align:center">
                    <label class="form-label">Pilih Waktu Latihan</label>
                </div>
                @if(!empty($slots))
                    <div class="d-flex flex-column gap-2">
                        @foreach($slots as $slot)
                            <div>
                                <input type="radio" class="btn-check" name="start_time" id="slot{{ $loop->index }}" value="{{ $slot['start'] }}">
                                <label class="slot-btn" for="slot{{ $loop->index }}">
                                    Pkl {{ \Carbon\Carbon::createFromFormat('H:i', $slot['start'])->format('H.i') }}
                                </label>
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="text-danger">Tidak ada slot waktu tersedia.</p>
                @endif
                @error('start_time')
                    <small class="text-danger">Jam mulai latihan wajib dipilih</small>
                @enderror
            </div>
...
<script>
    // Fungsi bantu: ubah waktu HH:MM ke menit
    function toMinutes(time) {
        const [h, m] = time.split(':').map(Number);
        return h * 60 + m;
    }

    // Fungsi bantu: ubah menit ke HH:MM
    ...
    // Fungsi: Membuat array waktu (interval 1 jam), lewati jam break
    function generateAvailableTimes(scheduleInfo) {
        const start = toMinutes(scheduleInfo.start_time);
        const end = toMinutes(scheduleInfo.end_time);
        const breakStart = toMinutes(scheduleInfo.start_break);
        const breakEnd = toMinutes(scheduleInfo.end_break);
    ...
    document.querySelectorAll('.btn-primary').forEach(btn => {
        btn.addEventListener('click', function () {
            const topupId = this.getAttribute('data-topup-id');
            const url = '{{ route("admin.members.detailschedule", ":id") }}'.replace(':id', topupId);

            fetch(url)
                .then(response => response.json())
                .then(data => {
                    const { schedules, available_times, training_range } = data;
                    const scheduleCardsContainer = document.getElementById('scheduleCards');
                    scheduleCardsContainer.innerHTML = '';

                    if (schedules.length === 0) {
                        scheduleCardsContainer.innerHTML = '<p class="text-center">Tidak ada jadwal ditemukan.</p>';
                        return;
                    }

                    schedules.forEach(schedule => {
                        const scheduleInfo = available_times.find(t => t.idtrainer === schedule.idtrainer);
                        let timeOptions = '';

                        if (scheduleInfo) {
                            const times = generateAvailableTimes(scheduleInfo);

                            // Tambahkan start_time dari jadwal sebagai opsi pertama
                            timeOptions = `<option value="${schedule.start_time}" selected>${schedule.start_time}</option>`;

                            // Tambahkan semua waktu dari jadwal trainer, kecuali yang sama dengan start_time (hindari duplikat)
                            timeOptions += times
                                .filter(time => time !== schedule.start_time)
                                .map(time => `<option value="${time}">${time}</option>`)
                                .join('');
                        } else {
                            timeOptions = '<option disabled>Tidak ada jam tersedia</option>';
                        }
                            ...
                            <div class="col-md-4 mb-3">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <form action="/admin/schedule/detail/${schedule.id}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="date" class="form-control" name="training_date"
                                                    value="${new Date(schedule.training_date).toISOString().slice(0, 10)}"
                                                    min="${new Date(training_range.start_training).toISOString().slice(0, 10)}"
                                                    max="${new Date(training_range.end_training).toISOString().slice(0, 10)}"
                                                    required>
                                            </div>
                        ...
                        scheduleCardsContainer.innerHTML += card;
                    });
                })
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: [
            @foreach($member_gym_schedule as $schedule)
                {
                    title: '{{ $schedule->name }} ( {{ $schedule->customer_name }} ) <br> ( {{ $schedule->start_time }} )', // Menggunakan <br> di title
